...
def _pad_cigar_sequences(cigar_sequences):
  if len(cigar_sequences) == 0:
    return np.zeros((0, 0), dtype=np.int64)

  max_len = max(len(seq) for seq in cigar_sequences)
  padded = np.zeros((len(cigar_sequences), max_len), dtype=np.int64)
  for idx, seq in enumerate(cigar_sequences):
    if len(seq) == 0:
      continue
    values = np.asarray(seq, dtype=np.int64)
...
def chioce_top18(tensor):
    ...
    tensor = torch.cat([tensor, torch.zeros((batch_size, window_size, 18), dtype=tensor.dtype, device=tensor.device)], dim=2)
    order = torch.argsort(torch.sum(tensor, dim=1), dim=1)[:, -18:]
    order = order.unsqueeze(1).expand(-1, window_size, -1)
    selected = torch.take_along_dim(tensor, order, dim=2)
    return selected.reshape(batch_size, window_size, 18, 1)
...
def cigarinfo(cigararray, refstartarray, start, end, reccount, cigarweight):
    cigararray = np.asarray(cigararray, dtype=np.int64)
    refstartarray = np.asarray(refstartarray, dtype=np.int64).reshape(-1)
    if cigararray.size == 0:
        return np.empty((0, 3), dtype=np.int64)

    active_mask = ~np.isin(cigararray[:, 0], [1, 4])
    adjusted = cigararray * active_mask.reshape(cigararray.shape[0], 1)
    a1 = adjusted[:, 1].reshape(reccount, cigarweight)
    ref_positions = np.cumsum(a1, axis=1) + refstartarray.reshape(-1, 1)
    expanded = np.concatenate([cigararray, ref_positions.reshape(cigararray.shape[0], 1)], axis=1)
    return expanded[(start <= expanded[:, -1]) & (expanded[:, -1] < end)]
...
def baseinfo(bamfile, contig, start, end):
    ...
    for AlignedSegment in bamfile.fetch(contig, start, end):
        if AlignedSegment.cigartuples is None:
            continue
        ...
        cigararray.append(np.asarray(AlignedSegment.cigartuples, dtype=np.int64).reshape(-1))
        readstartandend.append([AlignedSegment.reference_start-start, AlignedSegment.reference_end-start, AlignedSegment.mapping_quality, (1 - (AlignedSegment.query_alignment_length / read_length))**2])
    ...
    readstartandend = np.asarray(readstartandend, dtype=np.float32)
    cigararray = _pad_cigar_sequences(cigararray)
    reccount, cigarweight = cigararray.shape[0], int(cigararray.shape[1] / 2)
    cigararray = cigararray.reshape(int(cigararray.size / 2), 2)

    cigararray = cigarinfo(cigararray, readstartandend[:,:1], 0, end - start, reccount, cigarweight).astype('int64')
