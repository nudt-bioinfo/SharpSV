...
This branch keeps the original real-data release workflow intact and adds an experimental `sv5-direct` profile for five-class `DEL/INS/INV/TRA/DUP` detection. The default `release` profile and the original real-data pipeline remain unchanged.
...
For the five-class workflow added on this branch, switch to the custom profile:
...
python SharpSV.py \
  ...
  -workdir ./workdir \
  -processes 32 \
  -output ./SharpSV.sv5.vcf \
  --pipeline-profile sv5-direct
```
...
This branch ships compact inference checkpoints under `sv5_models/` so the branch stays GitHub-friendly while preserving the trained five-class models for end-to-end use. The final `-output` VCF is the refined `PASS` callset. The full unfiltered refined VCF remains in the workdir.

At the moment, the `sv5-direct` profile expects a source checkout because it invokes helper scripts from the repository `scripts/` directory.
...
For `--pipeline-profile sv5-direct`, the outputs are branch-specific:

- `stage-1`: BAM -> NPZ feature corpus reused from `-workdir`
- custom `stage-1` scoring -> `workdir/sv5_stage2_export/stage1_abnormal_windows.csv`
- custom five-class `stage-2` -> `workdir/sv5_stage2_export/stage2_window_predictions.csv`
- direct breakpoint refinement -> `workdir/sv5_breakpoint_refined/refined_events.csv`
- direct breakpoint refinement, all calls -> `workdir/sv5_breakpoint_refined/refined_events_all.vcf`
- direct breakpoint refinement, `PASS` calls -> `workdir/sv5_breakpoint_refined/refined_events_pass.vcf`
