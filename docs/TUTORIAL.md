...
2. run the bundled demo or a full BAM-to-VCF pipeline
3. rerun safely with built-in resume behavior
...
```bash
SharpSV -bamfilepath /path/to/sample.sorted.bam -fastapath /path/to/reference.fa --stage1-model /path/to/custom_stage1.ckpt --stage2-model /path/to/custom_stage2.ckpt -output ./SharpSV.vcf
```
